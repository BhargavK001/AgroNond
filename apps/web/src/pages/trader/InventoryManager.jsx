    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800">Inventory</h1>
        </div>
        
        {/* Stats - Responsive Scroll */}
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:overflow-visible">
          <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 shrink-0">
            ...
            <div>
              <p className="text-[10px] sm:text-xs text-slate-500">Stock</p>
              <p className="text-sm sm:text-base font-bold text-slate-800">{totalStock.toLocaleString()} kg</p>
            </div>
          </div>
          
          <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 shrink-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-emerald-100 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base font-bold text-emerald-600">₹{totalValue.toLocaleString('en-IN')}</p>
            </div>
          </div>
          
          {lowStockItems > 0 && (
            <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-red-50 rounded-xl sm:rounded-2xl border border-red-100 shrink-0">
              ...
              <div>
                <p className="text-[10px] sm:text-xs text-red-600">Low Stock</p>
                <p className="text-sm sm:text-base font-bold text-red-700">{lowStockItems} items</p>
              </div>
            </div>
          )}
        </div>
      ...
      >
        <div className="p-4 sm:p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <h2 className="text-base sm:text-lg font-bold text-slate-800">Stock Overview</h2>
          <div className="flex gap-2">
            <div className="flex p-1 bg-slate-100 rounded-xl sm:rounded-2xl flex-1 sm:flex-none">
              {['all', 'good', 'low'].map((filter) => (
                <button
                  key={filter}
                  onClick={() => setSelectedFilter(filter)}
                  className={`relative flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg sm:rounded-xl transition-all ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredInventory.map((item, index) => (
              <motion.div
                ...
                className="group"
              >
                <div className="p-4 sm:p-5 bg-slate-50 hover:bg-white rounded-xl sm:rounded-2xl border border-slate-100 hover:border-emerald-200 hover:shadow-lg transition-all">
                  <div className="flex items-start justify-between mb-3 sm:mb-4">
                    <div className="min-w-0">
                      <h3 className="font-bold text-sm sm:text-base text-slate-800 truncate group-hover:text-emerald-700 transition-colors">{item.crop}</h3>
                      <p className="text-[10px] sm:text-xs text-slate-400 font-mono mt-0.5">{item.batchId}</p>
                    </div>
                    <span className={`px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-medium shrink-0 ${
                      item.status === 'good' 
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <div>
                      <div className="flex justify-between text-xs sm:text-sm mb-1.5 sm:mb-2">
                        ...
                        <span className="font-medium text-slate-700">{item.quantity}/{item.maxQuantity} {item.unit}</span>
                      </div>
                    ...
                    <div className="flex justify-between items-center text-xs sm:text-sm pt-2 sm:pt-3 border-t border-slate-100">
                      <span className="text-slate-400 truncate">{item.location}</span>
                      <span className="font-bold text-emerald-600 shrink-0">₹{item.price}/kg</span>
                    </div>
                  </div>
                </div>
              </motion.div>
