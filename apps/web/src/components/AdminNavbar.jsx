          ...
          <div className="flex items-center gap-8">
            <Link to="/dashboard/admin" className="flex items-center gap-2 group shrink-0">
              ...
              <div className="hidden sm:block">
                ...
                <span className="block text-[10px] text-emerald-600 font-bold uppercase tracking-wider">Admin Panel</span>
              </div>
            </Link>
          </div>
            ...
            {navTabs.map((tab) => {
              ...
              return (
                <Link key={tab.path} to={tab.path} className={`relative px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${isActive ? 'text-white' : 'text-slate-500 hover:text-emerald-600'}`}>
                  {isActive && <motion.div layoutId="activeAdminNavTab" className="absolute inset-0 bg-gradient-to-r from-emerald-500 to-green-600 rounded-full shadow-md" transition={{ type: 'spring', stiffness: 400, damping: 30 }} />}
                        ...
                        <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                          <div className="p-6 bg-slate-50 border-b border-slate-100 text-center">
                            ...
                            <div className="flex items-center justify-center gap-1 mt-2 bg-emerald-50 py-1 px-3 rounded-full mx-auto w-fit border border-emerald-100">
                              <span className="text-xs font-bold text-emerald-700">{profile.adminId}</span>
                        ...
                        <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full">
                          ...
                          <div className="p-6 space-y-5 overflow-y-auto max-h-[400px]">
                            ...
                            <div className="space-y-4">
                              ...
                              <div><label className="block text-xs font-semibold text-slate-600 mb-1">Admin ID (Read Only)</label><div className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-500 font-medium flex justify-between">{profile.adminId}<BadgeCheck className="w-4 h-4 text-emerald-500" /></div></div>
                            </div>
                          </div>
