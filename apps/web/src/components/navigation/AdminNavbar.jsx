...
export default function AdminNavbar({ onMenuClick }) {
  ...
  const getPageTitle = () => {
    ...
    for (const [path, title] of Object.entries(PAGE_TITLES)) {
      if (location.pathname.startsWith(path) && path !== '/dashboard/admin') {
        return title;
      }
    }
    return 'Overview';
  ...
  const [profile, setProfile] = useState({
    name: user?.full_name || 'System Admin',
    phone: user?.phone || '9999999999',
    email: user?.email || 'admin@agronond.com',
    location: user?.location || 'Headquarters',
    role: 'Super Administrator',
    photo: user?.profile_picture || '',
    adminId: user?.customId || 'ADM-001',
    initials: 'SA'
  });
  ...
  useEffect(() => {
    if (user) {
      setProfile(prev => ({
        ...
        adminId: user.customId || prev.adminId,
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <div className="hidden md:block">
              <UniversalSearch placeholder="Search users, records..." />
            </div>
            <NotificationCenter />
                        ...
                        <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                          <div className="p-6 bg-gradient-to-br from-slate-50 to-slate-100 border-b border-slate-200 text-center">
                            <div className="relative inline-block mb-3">
                              {profile.photo ? <img src={profile.photo} alt="Profile" className="w-20 h-20 rounded-2xl object-cover shadow-lg ring-4 ring-white" /> : <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center text-2xl font-bold text-white shadow-lg ring-4 ring-white">{profile.initials}</div>}
                            ...
                            <div className="inline-flex items-center gap-1.5 mt-2 bg-emerald-100 py-1 px-3 rounded-full"><span className="text-xs font-bold text-emerald-700">{profile.adminId}</span></div>
                          </div>
