...
import { X, Phone, Store, FileText, IndianRupee, Package, Wheat, TrendingUp, Edit2, Trash2, Save, Download } from 'lucide-react';
...
export default function TraderDetailsModal({ isOpen, onClose, trader }) {
    ...
    const fetchHistory = async () => {
        setLoading(true);
        try {
            const response = await api.get(`/api/committee/traders/${trader._id}/history`);
    ...
    const handleSave = async () => {
        try {
            await api.patch(`/api/users/${trader._id}`, {
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this trader? This action cannot be undone.')) {
            try {
                await api.delete(`/api/users/${trader._id}`);
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed inset-0 m-auto max-w-5xl w-[95%] h-[90vh] bg-white rounded-3xl shadow-2xl z-50 overflow-hidden flex flex-col"
                    >
                        {/* Header */}
                                            ...
                                            <table className="w-full">
                                                <thead className="bg-slate-50 border-b border-slate-100">
                                                    <tr>
                                                        ...
                                                        <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-3">Farmer</th>
